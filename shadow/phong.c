...
/*   Updated: 2024/08/10 11:29:59 by yoshidakazu      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int phong_calc(t_scene *scene, t_vec3 screen_vec)
{
    ...
    //正規化による方向ベクトルの算出
    t_vec3 normal = vec3_norm(vec3_sub(intersection, *scene->object->center));
    ...
    int color_r = (int)(fmin(scene->object->rgb->r * brightness , 1.0) * 255);
    int color_g = (int)(fmin(scene->object->rgb->g * brightness , 1.0) * 255);
    int color_b = (int)(fmin(scene->object->rgb->b * brightness , 1.0) * 255);
